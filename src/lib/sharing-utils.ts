...
// Modify the createCaptionedVideo function to accept a style parameter
...
// Function to draw the standard caption style (your existing implementation)
    ...
    if (navigator.share) {
      ...
      if (mediaType !== 'text-only' && navigator.canShare) {
        try {
          ...
          const toastId = toast.loading('Preparing media for sharing...');
...
export const downloadPreview = async (
  ...
  try {
    ...
    if (mediaType === 'video') {
      ...
      toast.loading(`Creating video with ${captionStyle} style...`, { id: toastId });
      ...
      try {
        ...
        toast.success(`Video with ${captionStyle} captions downloaded!`, { id: toastId });
      } catch (captionError) {
        console.error('Error creating captioned video:', captionError);
        toast.error('Failed to create captioned video, downloading original instead', { id: toastId });
        
        // Fallback to original video, still using title-based filename
        await downloadOriginalVideo(video, filename || `${defaultFilename}-original.mp4`, toastId);
      }
    } else {
      ...
      toast.success('Content downloaded successfully!', { id: toastId });
    }
  } catch (error) {
    console.error('Download error:', error);
    toast.error('Failed to download content', { id: toastId });
    throw error;
  }
};
